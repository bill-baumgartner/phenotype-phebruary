...
FROM (SELECT distinct co.person_id,
      ...
      WHERE cohort.cohort_definition_id = @include_cohort_id AND
            ...
            co.condition_concept_id NOT IN (SELECT co2.condition_concept_id
                                            ...
                                            WHERE cohort2.cohort_definition_id = @exclude_cohort_id
                                            AND co2.condition_start_date <= cohort2.cohort_end_date AND co2.condition_end_date >= cohort2.cohort_start_date))
